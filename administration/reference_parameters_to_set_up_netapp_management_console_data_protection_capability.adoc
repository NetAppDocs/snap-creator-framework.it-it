...
NTAP_DFM_SNAPSHOT_FORMAT
 a| 
 a| 
Impostazione facoltativa per il formato delle copie Snapshot secondarie quando si utilizza la funzionalità di protezione dei dati della NetApp Management Console. Questa opzione richiede OnCommand 5.0 o versione successiva. Variabile obbligatoria:
...
* Tipo di conservazione: %R
* Etichetta del set di dati: %L
* Controller di archiviazione: %H
* Nome volume: %N
* Dati specifici dell'applicazione: %A


Se questa opzione non è impostata, verrà utilizzata la convenzione di denominazione predefinita per la funzionalità di protezione dei dati della NetApp Management Console. La convenzione di denominazione può essere impostata solo al momento della creazione del set di dati.
