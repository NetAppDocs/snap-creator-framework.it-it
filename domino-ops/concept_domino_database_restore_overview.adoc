...
In genere, i ripristini di volumi e i ripristini di singoli file hanno directory di destinazione diverse. È possibile specificare la directory di destinazione nel parametro Data Restore Directory nel file di configurazione di backup e ripristino (vedere il passaggio link:task_using_the_gui_to_create_a_configuration_file.html["attività utilizzando la gui per creare un file di configurazione"]):
