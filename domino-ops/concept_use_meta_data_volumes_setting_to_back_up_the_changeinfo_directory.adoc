...
Avresti già dovuto identificare il volume changeinfo come volume di cui eseguire il backup quando hai creato il file di configurazione. Per ulteriori informazioni, consulta il passaggio link:task_using_the_gui_to_create_a_configuration_file.html["attività utilizzando la gui per creare una configurazione"].
