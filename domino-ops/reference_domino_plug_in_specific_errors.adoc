...
NOTE: Per informazioni sugli errori generici di Snap Creator, vederelink:../administration/index.html["Guida all'amministrazione di Snap Creator Framework 4.3.3"] .
...
** Sugli host Windows, è necessario aggiungere il percorso di installazione di IBM Domino alla variabile di ambiente PATH. Per maggiori informazioni, vedere illink:https://docs.netapp.com/us-en/snap-creator-framework/installation/task_configure_windows_specific_settings_adding_path_to_the_environment_variables.html["Guida all'installazione Snap Creator Framework - Aggiunta del percorso alle variabili di ambiente"] .
...
Per impostazione predefinita, un'operazione di backup non andrà a buon fine se uno o più database si trovano in uno stato incoerente o danneggiato. È possibile forzare il plug-in a procedere con le operazioni di backup nonostante gli errori, come descritto nel passaggio link:task_using_the_gui_to_create_a_configuration_file.html["attività utilizzando la gui per creare un file di configurazione"].
...
Il plug-in Domino non funziona con SNAP_TIMESTAMP_ONLY = N. SNAP_TIMESTAMP_ONLY deve essere impostato su Y.
 a| 
Per ulteriori informazioni, vedere il passaggio link:task_using_the_gui_to_create_a_configuration_file.html["attività_utilizzo_della_GUI_per_creare_un_file_di_configurazione"].
